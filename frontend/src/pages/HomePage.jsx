...
const HomePage = () => {
    ...
    const handleCandidateIdAccess = async () => {
        ...
        try {
            ...
            // Access session using candidate ID from database
            ...
            console.log('📡 HTTP Status:', response.status, 'Content-Type:', response.headers.get('content-type'))
            ...
            try {
                ...
                setSessionError('Backend returned invalid JSON response. Check server logs.')
                setIsLoadingSession(false)
                return
            }

            console.log('🔍 Checking data.success:', data.success, 'Type:', typeof data.success)
            ...
            if (data.success === true) {
                ...
                if (!data.session || !data.session.sessionId) {
                    console.error('❌ Missing session.sessionId in response:', data.session)
                    setSessionError('Invalid session data from backend')
                    setIsLoadingSession(false)
                    return
                }
                ...
                const sessionInfo = {
                    ...
                    companyName: data.session.companyName || 'Your Company',
                ...
                console.error('❌ Backend returned success:', data.success, 'Full response:', data)
