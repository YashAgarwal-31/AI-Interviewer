...
function validateProductionConfig() {
  ...
  if (!(process.env.FRONTEND_URL || process.env.PRODUCTION_FRONTEND_URL || process.env.CORS_ORIGINS)) {
...
// Existing recruiter/admin mutations are automatically recorded after authorization attaches req.actor.
app.use((req, res, next) => {
...
async function connectDatabase() {
  ...
  const maxPoolSize = Number.isFinite(configuredPoolSize)
    ? Math.min(50, Math.max(5, configuredPoolSize))
    : 20;
...
function createOpenAIClient() {
  if (!process.env.OPENAI_API_KEY) {
    console.warn('OPENAI_API_KEY is not configured. Interview questions will use local fallbacks.');
    return null;
  }
  ...
  try {
    return new OpenAI({
      ...
      timeout: Math.max(5000, Number(process.env.OPENAI_TIMEOUT_MS) || 45000),
      maxRetries: Math.min(5, Math.max(0, Number(process.env.OPENAI_MAX_RETRIES) || 2))
    });
  } catch (error) {
...
app.get('/api/health', (req, res) => {
  ...
  return res.status(healthy ? 200 : 503).json({
    ...
    version: process.env.RENDER_GIT_COMMIT?.slice(0, 12) || process.env.APP_VERSION || null,
    uptimeSeconds: Math.round(process.uptime()),
    timestamp: new Date().toISOString()
  });
});

app.get('/api/db-health', requireAdmin, (req, res) => {
...
async function startServer() {
  ...
  app.use((req, res) => {
    return res.status(404).json({ success: false, error: 'API route not found', requestId: req.requestId });
  });

  app.use((error, req, res, next) => {
    if (res.headersSent) return next(error);
    if (error?.message === 'Origin is not allowed by CORS') {
      return res.status(403).json({ success: false, error: 'Origin is not allowed', requestId: req.requestId });
    }
    if (error?.type === 'entity.too.large') {
      return res.status(413).json({ success: false, error: 'Request payload is too large', requestId: req.requestId });
    }
  ...
  server = app.listen(port, () => {
    console.log(`AI Interviewer backend listening on port ${port}`);
