...
if (process.env.OPENAI_API_KEY) {
    ...
    console.log('ℹ️ OPENAI_API_KEY not provided - running in demo mode');
}
...
// Quick demo endpoint for testing without MongoDB
app.post('/api/sessions/access-demo/:candidateId', (req, res) => {
    try {
        console.log('🎮 [DEMO] Demo endpoint called with ID:', req.params.candidateId);
        
        const { candidateId } = req.params;
        const sessionId = `demo-session-${Date.now()}`;
        const accessToken = `token-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
        ...
        const demoData = {
            ...
            accessToken,
            initialMessage: `Hello! Welcome to your AI Technical Interview for the Full Stack Developer position. This is a demo session. Let's begin!`,
            questions: [
                'Tell me about your experience with React and Redux.',
                ...
                'How do you handle testing in your development workflow?'
            ],
            codingTasks: [
                {
                    id: 'task-1',
                ...
                },
                {
                    id: 'task-2',
                    title: 'API Design',
                    description: 'Design a RESTful API for a task management application',
                    difficulty: 'Medium',
                    language: 'JavaScript'
                }
            ]
        ...
        console.log('✅ [DEMO] Sending demo data:', JSON.stringify(demoData, null, 2));
        res.json(demoData);
